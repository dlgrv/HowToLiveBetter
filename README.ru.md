...
603 пунктов, в каждом написано, что тратится, что взамен и насколько твёрдые доказательства; источники — только журнальные статьи и официальные документы.
...
- **Хочу только самые твёрдые выводы**: отметьте на странице поиска уровень доказательности A — останутся 408 пунктов с конкретными цифрами, из метаанализов или крупных исследований.
...
Из всех 603 пунктов книги 408 имеют уровень A, 146 — уровень B и 49 — уровень C; кроме того, у 55 пунктов отмечено «спорно», а в 35 местах стоит пометка «TODO: проверить». У оспариваемых пунктов уровней A/B стоит пометка «спорно» и приведены аргументы противоположной стороны. Источники — только первоисточники (журнальные статьи с DOI или ссылкой PubMed либо отчёты официальных организаций: ВОЗ, CDC, государственное статистическое бюро и т.п.); пересказы из вторых рук не цитируются. Ненадёжные числа помечаются «TODO: проверить».
...
Из всех 603 пунктов книги: очень высокое соотношение у 104 пунктов (17%), высокое — у 276 (46%), обычное — у 223 (37%). Утолщение среднего уровня сделано намеренно: в основе лежит лишь трёхуровневая оценка порядка результата, а более мелкое деление было бы притворной точностью.
