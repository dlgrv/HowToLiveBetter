...
608 пунктов, в каждом написано, что тратится, что взамен и насколько твёрдые доказательства; источники — только журнальные статьи и официальные документы.
...
[![Пунктов](https://img.shields.io/badge/%D0%9F%D1%83%D0%BD%D0%BA%D1%82%D0%BE%D0%B2-608-18794e?style=flat-square)](#оглавление)
[![Уровни доказательности](https://img.shields.io/badge/%D0%A3%D1%80%D0%BE%D0%B2%D0%BD%D0%B8%20%D0%B4%D0%BE%D0%BA%D0%B0%D0%B7%D0%B0%D1%82%D0%B5%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8-A%20410%20%C2%B7%20B%20149%20%C2%B7%20C%2049-915930?style=flat-square)](#уровни-доказательности)
[![Первоисточники](https://img.shields.io/badge/%D0%9F%D0%B5%D1%80%D0%B2%D0%BE%D0%B8%D1%81%D1%82%D0%BE%D1%87%D0%BD%D0%B8%D0%BA%D0%B8-1281%20%D1%81%D1%81%D1%8B%D0%BB%D0%BA%D0%B0-565a5f?style=flat-square)](docs/核实记录/)
...
- **Хочу только самые твёрдые выводы**: отметьте на странице поиска уровень доказательности A — останутся 410 пунктов с конкретными цифрами, из метаанализов или крупных исследований.
...
Из всех 608 пунктов книги 410 имеют уровень A, 149 — уровень B и 49 — уровень C; кроме того, у 56 пунктов отмечено «спорно», а в 29 местах стоит пометка «TODO: проверить». У оспариваемых пунктов уровней A/B стоит пометка «спорно» и приведены аргументы противоположной стороны. Источники — только первоисточники (журнальные статьи с DOI или ссылкой PubMed либо отчёты официальных организаций: ВОЗ, CDC, государственное статистическое бюро и т.п.); пересказы из вторых рук не цитируются. Ненадёжные числа помечаются «TODO: проверить».
...
Из всех 608 пунктов книги: очень высокое соотношение у 104 пунктов (17%), высокое — у 276 (45%), обычное — у 228 (38%). Утолщение среднего уровня сделано намеренно: в основе лежит лишь трёхуровневая оценка порядка результата, а более мелкое деление было бы притворной точностью.
...
33. [Как жить после инвалидности](book/ru/33-Как-жить-после-инвалидности.md): первое десятилетие после тяжёлой инвалидности — период высокого риска суицида; добровольность госпитализации при психических расстройствах и два исключения; противопролежневые подушки для колясочников; технические средства реабилитации и переустройство жилья через федерацию инвалидов; оформление инвалидности и пособия; дискриминация при поступлении в школу и приёме на работу — и как её оспорить

Внутри каждого раздела пункты идут по убыванию соотношения «результат/затраты». Заголовки разделов вроде «Не умирайте рано» или «Не тратьте время впустую» описывают результат, который раздел предотвращает; делать пункт или не делать — решайте по заголовку самого пункта. Длинные статьи см. также [Домашний аварийный комплект](docs/ru/Домашний-аварийный-комплект.md), [Какие документы нужны для запуска площадки](docs/ru/Какие-документы-нужны-для-запуска-площадки.md), [Выгоден ли брак](docs/ru/Выгоден-ли-брак.md) и [Стоит ли останавливаться, если с незнакомцем случилась беда](docs/ru/Стоит-ли-помогать-незнакомцу.md). Процесс проверки каждого источника записан в [Записи проверки](docs/核实记录/).
...
Текст разбит на 33 файла по главам в каталоге [book/](book/): нажмите на название раздела в оглавлении выше. Разбивка сделана потому, что единый файл превысил лимит GitHub в 512 KB на рендеринг Markdown и последние разделы перестали бы отображаться; [страница онлайн-поиска](https://dlgrv.github.io/HowToLiveBetter/ru/) читает эти файлы вместе, порядок работы тот же.
