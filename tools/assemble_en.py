...
for i in range(1, meta["items"] + 1):
    ...
    for j, l in enumerate(lines):
        if l.strip() == "§SRC§":
            # find where to insert: after the LAST '- Evidence grade'
            # already emitted, before a following '- Примечания' if present
            k = len(out_lines)
            while k > 0 and not out_lines[k-1].startswith("- Evidence grade"):
                k -= 1
            if k == 0:
...
ts = [x.split(":", 1)[1].strip() for x in tl if re.match(r"^- Sources:", x)]
if len(ss) != len(ts):
...
for idx, l in enumerate(tl, 1):
    if hanzi(l) and not (l.startswith("- Sources:") or "成本标签" in l
                         or "](../" in l or idx <= 4):
        zh_lines_out += 1
