...
for i in range(1, meta["items"] + 1):
    ...
    src_ru = ["- Fuentes: " + l.split("：", 1)[1] if l.startswith("- 来源：") else l
              for l in src]
    ...
    for j, l in enumerate(lines):
        if l.strip() == "§SRC§":
            # find where to insert: after the LAST '- Nivel de evidencia'
            # already emitted, before a following '- Примечания' if present
            k = len(out_lines)
            while k > 0 and not out_lines[k-1].startswith("- Nivel de evidencia"):
                k -= 1
            if k == 0:
...
ts = [x.split(":", 1)[1].strip() for x in tl if re.match(r"^- Fuentes:", x)]
if len(ss) != len(ts):
...
for idx, l in enumerate(tl, 1):
    if hanzi(l) and not (l.startswith("- Fuentes:") or "成本标签" in l
                         or "](../" in l or idx <= 4):
        zh_lines_out += 1
